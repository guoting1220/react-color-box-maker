...
it("can add a new box", function() {
  ...
  expect(boxList.getAllByText("X")).toHaveLength(2);

  addBox(boxList);
 
  expect(boxList.getAllByText("X")).toHaveLength(3);

});

it("can remove a box", function() {
  const boxList = render(<BoxList />);
  const removeButton = boxList.getAllByText("X")[0];

  // click the remove button and the box should be gone
  fireEvent.click(removeButton);
  expect(boxList.getAllByText("X")).toHaveLength(1);
});
